                             ...
                             LibertyPort *from_to_port)
{
  for (TimingArcSet *arc_set : cell->timingArcSets()) {
    ...
    if ((from_to_port == nullptr || from_port == from_to_port || to_port == from_to_port) && from_port) {
      ...
      if (from_pin && to_pin) {
        ...
        if (from_vertex) {
          ...
          if (from_bidirect_drvr_vertex && to_vertex) {
            ...
            // instance.
            Edge *edge = makeEdge(from_bidirect_drvr_vertex, to_vertex, arc_set);
                             ...
                             PinSet &visited_drvrs)
{
  ...
  while (pin_iter->hasNext()) {
    Pin *pin = pin_iter->next();
    if (network_->isDriver(pin) && !visited_drvrs.hasKey(pin))
                            ...
                            PinSet &visited_drvrs)
{
  ...
  if (isIsolatedNet(drvrs, loads)) {
    for (auto drvr_pin : drvrs) {
      ...
      debugPrint(debug_, "graph", 1, "ignoring isolated driver %s", network_->pathName(drvr_pin));
    }
    return;
  }
                     ...
                     PinSeq &loads) const
{
  ...
  for (auto drvr_pin : drvrs) {
    ...
    if (network_->isTopLevelPort(drvr_pin) || drvr_vertex->hasFanin())
      return false;
  }
  // Check for fanout on the load pins.
  for (auto load_pin : loads) {
    Vertex *load_vertex = pinLoadVertex(load_pin);
    if (load_vertex->hasFanout() || load_vertex->hasChecks()) {
      return false;
    }
  }
  return true;
...
{
  return vertex->in_edges_ && edge(vertex->in_edges_)->vertex_in_link_ == 0;
}
  ...
  while (edge_iter.hasNext()) {
    ...
    if (edge1->from(this) == in_vertex) {
      ...
      for (TimingArc *arc1 : arc_set->arcs()) {
        if (arc1->fromEdge()->asRiseFall() == in_rf && arc1->toEdge()->asRiseFall() == drvr_rf) {
          edge = edge1;
          arc = arc1;
          return;
        }
...
Graph::wireDelayAnnotated(Edge *edge,
                          ...
                          DcalcAPIndex ap_index) const
{
  size_t index = (edge->arcDelays() + TimingArcSet::wireArcIndex(rf)) * ap_count_ + ap_index;
  if (index >= arc_delay_annotated_.size())
...
Vertex::isDriver(const Network *network) const
{
  ...
  return ((top_level_port && (dir->isInput() || (dir->isBidirect() && is_bidirect_drvr_))) || (!top_level_port && (dir->isOutput() || dir->isTristate() || (dir->isBidirect() && is_bidirect_drvr_) || dir->isInternal())));
}
...
Edge::isDisabledConstraint() const
{
  ...
  return is_disabled_constraint_ || arc_set_->isDisabledConstraint()
      // set_disable_timing cell does not disable timing checks.
      || (!(role->isTimingCheck() || is_wire) && arc_set_->libertyCell()->isDisabledConstraint()) || (!is_wire && arc_set_->from()->isDisabledConstraint()) || (!is_wire && arc_set_->to()->isDisabledConstraint());
}
