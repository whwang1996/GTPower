...
VcdReader::parseVar()
{
  ...
  if (tokens.size() == 4 || tokens.size() == 5) {
    ...
    if (type == VcdVarType::unknown)
      report_->fileWarn(1370, filename_, stmt_line_, "Unknown variable type %s.", type_name.c_str());
    else {
      ...
      replaceAll(tokens[3], "/", "\\/");  // convert / to \\/ to keep the same with verilog parser
      name += tokens[3];
...
{
  NEeventVal n_cur_interval_event = 0;
  std::vector<std::pair<VcdEventTime, VcdEventTime>> time_intervals;
  string token = getToken();
  while (!token.empty()) {
    NEeventVal n_parsed_event = 0;
    ...
    else if (char0 == '0' || char0 == '1' || char0 == 'X' || char0 == 'U' || char0 == 'Z') {
      string id = token.substr(1);
      if (!vcd_->varIdValid(id))
        report_->fileError(805, filename_, stmt_line_, "unknown variable %s", id.c_str());
      vcd_->varAppendValue(id, time_, char0);
      n_parsed_event = 1;
    }
    else if (char0 == 'B') {
      char char1 = toupper(token[1]);
      if (char1 == 'X' || char1 == 'U' || char1 == 'Z') {
        string id = getToken();
        if (!vcd_->varIdValid(id))
          report_->fileError(806, filename_, stmt_line_, "unknown variable %s", id.c_str());
        // Bus mixed 0/1/X/U not supported.
        LOG_WARN << "Bus mixed 0/1/X/U not supported for " << token << " of " << id << ", use 0 instead.";
        vcd_->varAppendBusValue(id, time_, 0, static_cast<int>(token.size() - 1));
      }
      else {
        char *end;
        int64_t bus_value = strtol(token.c_str() + 1, &end, 2);
        string id = getToken();
        if (!vcd_->varIdValid(id))
          report_->fileError(807, filename_, stmt_line_, "unknown variable %s", id.c_str());
        else
          vcd_->varAppendBusValue(id, time_, bus_value, static_cast<int>(token.size() - 1));
      }
      n_parsed_event = token.size() - 1;
    }
    token = getToken();

    n_cur_interval_event += n_parsed_event;
    if (n_cur_interval_event > G_CONFIG.nums.max_event_num) {
      time_intervals.emplace_back(time_intervals.size() == 0 ? 0 : time_intervals.at(time_intervals.size() - 1).second, time_);
      n_cur_interval_event = 0;
    }
  }
  vcd_->setTimeMax(time_);
  time_intervals.emplace_back(time_intervals.size() == 0 ? 0 : time_intervals.at(time_intervals.size() - 1).second, time_ + 1);  // wrap up the last interval
                ...
                Report *report)
{
  ...
  for (VcdVar *var : vcd.vars()) {
    string line;
    stringPrint(line, " %-*s", static_cast<int>(max_var_name_length), var->name().c_str());
          ...
          if (var->width() == 1) {
            if (value == '0' || value == '1') {
              for (int z = 0; z < zoom; z++) {
                if (z == 0 && value != prev_value && (prev_value == '0' || prev_value == '1'))
                  line += (prev_value == '1') ? "╲" : "╱";
                else
                  line += (value == '1') ? "▔" : "▁";
              }
