...
typedef unordered_map<string, VcdVar *> VcdNameMap;
...
public:
  ...
  const VcdValues &values(const VcdVar *var) const;
  ...
  const vector<VcdVar*> &vars() const { return vars_; }
  ...
  const VcdNameMap& VarNameMap() const { return var_name_map_; }
  const unordered_map<string, VcdValues>& idValuesMap() const { return id_values_map_; }
  bool varIdValid(const string &id) const;
  ...
  void varAppendBusValue(const string &id,
                         ...
                         int64_t bus_value,
                         int bus_width);
  const std::vector<std::pair<VcdEventTime, VcdEventTime>>& timeIntervals() { return time_intervals_; }
  void setTimeIntervals(const std::vector<std::pair<VcdEventTime, VcdEventTime>>& time_intervals);
...
private:
  ...
  int max_var_width_;
  unordered_map<string, VcdValues> id_values_map_;
  VcdTime min_delta_time_;
  VcdTime time_max_;
  std::vector<std::pair<VcdEventTime, VcdEventTime>> time_intervals_;  // for time slicing
};
...
public:
  ...
  int busWidth() const { return bus_width_; }
