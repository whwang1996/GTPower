...
{
  debugPrint(debug_, "delay_calc", 2, "delay invalid %s", vertex->name(sdc_network_));
  if (graph_ && incremental_) {
                             ...
                             ArcDelayCalc *arc_delay_calc)
{
  ...
  debugPrint(debug_, "delay_calc", 2, "seed driver slew %s", drvr_vertex->name(sdc_network_));
  InputDrive *drive = 0;
  ...
  for (auto rf : RiseFall::range()) {
    for (auto dcalc_ap : corners_->dcalcAnalysisPts()) {
      if (drive) {
        ...
        float *from_slews;
        drive->driveCell(rf, cnst_min_max, drvr_cell, from_port, from_slews, to_port);
        if (drvr_cell) {
          if (from_port == nullptr)
            from_port = driveCellDefaultFromPort(drvr_cell, to_port);
          findInputDriverDelay(drvr_cell, drvr_pin, drvr_vertex, rf, from_port, from_slews, to_port, dcalc_ap);
        }
        else
          seedNoDrvrCellSlew(drvr_vertex, drvr_pin, rf, drive, dcalc_ap, arc_delay_calc);
      }
      else
                                   ...
                                   ArcDelayCalc *arc_delay_calc)
{
  ...
  float load_cap;
  parasiticLoad(drvr_pin, rf, dcalc_ap, nullptr, arc_delay_calc, load_cap, parasitic);
  if (exists) {
  ...
  ArcDcalcResult dcalc_result =
      arc_delay_calc->inputPortDelay(drvr_pin, delayAsFloat(slew), rf, parasitic, load_pin_index_map, dcalc_ap);
  annotateLoadDelays(drvr_vertex, rf, dcalc_result, load_pin_index_map, drive_delay, false, dcalc_ap);
                               ...
                               ArcDelayCalc *arc_delay_calc)
{
  ...
  ArcDcalcResult dcalc_result =
      arc_delay_calc->inputPortDelay(drvr_pin, delayAsFloat(slew), rf, parasitic, load_pin_index_map, dcalc_ap);
  annotateLoadDelays(drvr_vertex, rf, dcalc_result, load_pin_index_map, delay_zero, false, dcalc_ap);
...
GraphDelayCalc::seedLoadSlew(Vertex *vertex)
{
  ...
  debugPrint(debug_, "delay_calc", 2, "seed load slew %s", vertex->name(sdc_network_));
                                         ...
                                         const LibertyPort *to_port)
{
  ...
  for (TimingArcSet *arc_set : cell->timingArcSets(nullptr, to_port)) {
    ...
    if (from_port == nullptr || set_from_port_index < from_port_index) {
...
{
  debugPrint(debug_, "delay_calc", 2, "  driver cell %s %s", drvr_cell->name(), rf->asString());
...
GraphDelayCalc::findInputArcDelay(const Pin *drvr_pin,
                                  ...
                                  const DcalcAnalysisPt *dcalc_ap)
{
  debugPrint(debug_, "delay_calc", 3, "  %s %s -> %s %s (%s)", arc->from()->name(), arc->fromEdge()->asString(), arc->to()->name(), arc->toEdge()->asString(), arc->role()->asString());
  const RiseFall *drvr_rf = arc->toEdge()->asRiseFall();
  if (drvr_rf) {
    DcalcAPIndex ap_index = dcalc_ap->index();
    const Parasitic *parasitic;
    float load_cap;
    parasiticLoad(drvr_pin, drvr_rf, dcalc_ap, nullptr, arc_delay_calc_, load_cap, parasitic);

    LoadPinIndexMap load_pin_index_map = makeLoadPinIndexMap(drvr_vertex);
    ArcDcalcResult intrinsic_result =
        arc_delay_calc_->gateDelay(drvr_pin, arc, Slew(from_slew), 0.0, nullptr, load_pin_index_map, dcalc_ap);
    ArcDelay intrinsic_delay = intrinsic_result.gateDelay();

    ArcDcalcResult gate_result = arc_delay_calc_->gateDelay(drvr_pin, arc, Slew(from_slew), load_cap, parasitic, load_pin_index_map, dcalc_ap);
    ...
    debugPrint(debug_, "delay_calc", 3, "    gate delay = %s intrinsic = %s slew = %s", delayAsString(gate_delay, this), delayAsString(intrinsic_delay, this), delayAsString(gate_slew, this));
    graph_->setSlew(drvr_vertex, drvr_rf, ap_index, gate_slew);
    annotateLoadDelays(drvr_vertex, drvr_rf, gate_result, load_pin_index_map, load_delay, false, dcalc_ap);
    arc_delay_calc_->finishDrvrPin();
  }
}
...
{
  const Pin *pin = vertex->pin();
  debugPrint(debug_, "delay_calc", 2, "find delays %s (%s)", vertex->name(sdc_network_), network_->cellName(network_->instance(pin)));
  if (vertex->isRoot()) {
    ...
    // annotateLoadDelays.
    else if (vertex->isBidirectDriver() && network_->isTopLevelPort(pin))
      seedRootSlew(vertex, arc_delay_calc);
  }
}
                                 ...
                                 ArcDelayCalc *arc_delay_calc)
{
  ...
  if (multi_drvr == nullptr || (multi_drvr && (!multi_drvr->parallelGates(network_) || drvr_vertex == multi_drvr->dcalcDrvr()))) {
    ...
    while (edge_iter.hasNext()) {
      ...
      if (edge->isWire()) {
        ...
        if (isLeafDriver(drvr_pin, network_)) {
          debugPrint(debug_, "delay_calc", 3, " %s", network_->pathName(drvr_pin));
          multi_drvr_net_map_[drvr] = multi_drvr;
          drvr_vertices.push_back(drvr);
          Level drvr_level = drvr->level();
          if (max_drvr == nullptr || drvr_level > max_drvr_level) {
            max_drvr = drvr;
                                  ...
                                  ArcDelayCalc *arc_delay_calc)
{
  ...
  if (multi_drvr && multi_drvr->parallelGates(network_)) {
  ...
  while (edge_iter.hasNext()) {
    ...
    if (search_pred_->searchFrom(from_vertex) && search_pred_->searchThru(edge) && !edge->role()->isLatchDtoQ())
      delay_changed |= findDriverEdgeDelays(drvr_vertex, multi_drvr, edge, arc_delay_calc, delay_exists);
  }
  for (auto rf : RiseFall::range()) {
...
GraphDelayCalc::findLatchEdgeDelays(Edge *edge)
{
  ...
  debugPrint(debug_, "delay_calc", 2, "find latch D->Q %s", sdc_network_->pathName(drvr_inst));
  array<bool, RiseFall::index_count> delay_exists = {false, false};
  bool delay_changed = findDriverEdgeDelays(drvr_vertex, nullptr, edge, arc_delay_calc_, delay_exists);
  if (delay_changed && observer_)
    observer_->delayChangedTo(drvr_vertex);
}
                                     ...
                                     array<bool, RiseFall::index_count> &delay_exists)
{
  ...
  for (auto dcalc_ap : corners_->dcalcAnalysisPts()) {
    for (const TimingArc *arc : arc_set->arcs()) {
      delay_changed |= findDriverArcDelays(drvr_vertex, multi_drvr, edge, arc, load_pin_index_map, dcalc_ap, arc_delay_calc);
                                    ...
                                    ArcDelayCalc *arc_delay_calc)
{
  MultiDrvrNet *multi_drvr = multiDrvrNet(drvr_vertex);
  LoadPinIndexMap load_pin_index_map = makeLoadPinIndexMap(drvr_vertex);
  findDriverArcDelays(drvr_vertex, multi_drvr, edge, arc, load_pin_index_map, dcalc_ap, arc_delay_calc);
}
                                    ...
                                    ArcDelayCalc *arc_delay_calc)
{
  ...
  if (from_rf && drvr_rf) {
    ...
    const Parasitic *parasitic;
    float load_cap;
    parasiticLoad(drvr_pin, drvr_rf, dcalc_ap, multi_drvr, arc_delay_calc, load_cap, parasitic);

    if (multi_drvr && multi_drvr->parallelGates(network_)) {
      ArcDcalcArgSeq dcalc_args = makeArcDcalcArgs(drvr_vertex, multi_drvr, edge, arc, dcalc_ap, arc_delay_calc);
      ...
      for (size_t drvr_idx = 0; drvr_idx < dcalc_args.size(); drvr_idx++) {
        ...
        delay_changed |= annotateDelaysSlews(dcalc_arg.edge(), dcalc_arg.arc(), dcalc_result, load_pin_index_map, dcalc_ap);
      }
    }
    else {
      Vertex *from_vertex = edge->from(graph_);
      const Slew in_slew = edgeFromSlew(from_vertex, from_rf, edge, dcalc_ap);
      ArcDcalcResult dcalc_result = arc_delay_calc->gateDelay(drvr_pin, arc, in_slew, load_cap, parasitic, load_pin_index_map, dcalc_ap);
      delay_changed |= annotateDelaysSlews(edge, arc, dcalc_result, load_pin_index_map, dcalc_ap);
    }
    arc_delay_calc->finishDrvrPin();
  }
  return delay_changed;
                                 ...
                                 ArcDelayCalc *arc_delay_calc)
{
  ...
  for (auto drvr_vertex1 : multi_drvr->drvrs()) {
    ...
    if (edge1) {
      ...
      float load_cap;
      const Parasitic *parasitic;
      parasiticLoad(drvr_pin1, drvr_rf, dcalc_ap, multi_drvr, arc_delay_calc, load_cap, parasitic);
      dcalc_args.emplace_back(from_pin, drvr_pin1, edge1, arc1, in_slew, load_cap, parasitic);
    }
  }
  return dcalc_args;
}
  ...
  if (vertex_cell == drvr_cell) {
    ...
    while (edge_iter.hasNext()) {
      ...
      for (TimingArc *arc1 : edge->timingArcSet()->arcs()) {
        if (arc1->fromEdge() == drvr_arc->fromEdge() && arc1->toEdge() == drvr_arc->toEdge()) {
          arc = arc1;
          return;
        }
      }
...
GraphDelayCalc::annotateDelaysSlews(Edge *edge,
                                    ...
                                    const DcalcAnalysisPt *dcalc_ap)
{
  bool delay_changed = annotateDelaySlew(edge, arc, dcalc_result.gateDelay(), dcalc_result.drvrSlew(), dcalc_ap);
  if (!edge->role()->isLatchDtoQ()) {
    Vertex *drvr_vertex = edge->to(graph_);
    delay_changed |= annotateLoadDelays(drvr_vertex, arc->toEdge()->asRiseFall(), dcalc_result, load_pin_index_map, delay_zero, true, dcalc_ap);
  }
  return delay_changed;
}
                                  ...
                                  const DcalcAnalysisPt *dcalc_ap)
{
  bool delay_changed = false;
  DcalcAPIndex ap_index = dcalc_ap->index();
  debugPrint(debug_, "delay_calc", 3, "  %s %s -> %s %s (%s) corner:%s/%s", arc->from()->name(), arc->fromEdge()->asString(), arc->to()->name(), arc->toEdge()->asString(), arc->role()->asString(), dcalc_ap->corner()->name(), dcalc_ap->delayMinMax()->asString());
  debugPrint(debug_, "delay_calc", 3, "    gate delay = %s slew = %s", delayAsString(gate_delay, this), delayAsString(gate_slew, this));
  Vertex *drvr_vertex = edge->to(graph_);
  const RiseFall *drvr_rf = arc->toEdge()->asRiseFall();
  // Merge slews.
  const Slew &drvr_slew = graph_->slew(drvr_vertex, drvr_rf, ap_index);
  const MinMax *slew_min_max = dcalc_ap->slewMinMax();
  if (delayGreater(gate_slew, drvr_slew, slew_min_max, this) && !drvr_vertex->slewAnnotated(drvr_rf, slew_min_max) && !edge->role()->isLatchDtoQ())
    graph_->setSlew(drvr_vertex, drvr_rf, ap_index, gate_slew);
  if (!graph_->arcDelayAnnotated(edge, arc, ap_index)) {
    ...
    if (prev_gate_delay1 == 0.0 || (abs(gate_delay1 - prev_gate_delay1) / prev_gate_delay1 > incremental_delay_tolerance_))
      delay_changed = true;
                                   ...
                                   const DcalcAnalysisPt *dcalc_ap)
{
  ...
  while (edge_iter.hasNext()) {
    ...
    if (wire_edge->isWire()) {
      ...
      debugPrint(debug_, "delay_calc", 3, "    %s load delay = %s slew = %s", load_vertex->name(sdc_network_), delayAsString(wire_delay, this), delayAsString(load_slew, this));
                        ...
                        float &wire_cap) const
{
  ...
  const Parasitic *parasitic;
  parasiticLoad(drvr_pin, rf, dcalc_ap, multi_drvr, arc_delay_calc_, pin_cap, wire_cap, parasitic);
  arc_delay_calc_->finishDrvrPin();
}
                        ...
                        ArcDelayCalc *arc_delay_calc) const
{
  const Parasitic *parasitic;
  float pin_cap, wire_cap;
  parasiticLoad(drvr_pin, rf, dcalc_ap, nullptr, arc_delay_calc, pin_cap, wire_cap, parasitic);
  return pin_cap + wire_cap;
}
...
{
  float pin_cap, wire_cap;
  parasiticLoad(drvr_pin, rf, dcalc_ap, multi_drvr, arc_delay_calc, pin_cap, wire_cap, parasitic);
                              ...
                              const Parasitic *&parasitic) const
{
  bool has_net_load;
  float fanout;
  netCaps(drvr_pin, rf, dcalc_ap, multi_drvr, pin_cap, wire_cap, fanout, has_net_load);
...
{
  if (multi_drvr)
    multi_drvr->netCaps(rf, dcalc_ap, pin_cap, wire_cap, fanout, has_net_load);
  else {
    const Corner *corner = dcalc_ap->corner();
    const MinMax *min_max = dcalc_ap->constraintMinMax();
    // Find pin and external pin/wire capacitance.
    sdc_->connectedCap(drvr_pin, rf, corner, min_max, pin_cap, wire_cap, fanout, has_net_load);
  }
}
...
GraphDelayCalc::edgeFromSlew(const Vertex *from_vertex,
                             ...
                             const DcalcAnalysisPt *dcalc_ap)
{
  if (role->genericRole() == TimingRole::regClkToQ() && clk_network_->isIdealClock(from_vertex->pin()))
    return clk_network_->idealClkSlew(from_vertex->pin(), from_rf, dcalc_ap->slewMinMax());
  else
                                    ...
                                    ArcDelayCalc *arc_delay_calc)
{
  ...
  debugPrint(debug_, "delay_calc", 2, "find check %s %s -> %s", sdc_network_->pathName(inst), network_->portName(from_vertex->pin()), network_->portName(to_pin));
  bool delay_changed = false;
  for (TimingArc *arc : arc_set->arcs()) {
    ...
    if (from_rf && to_rf) {
      ...
      for (auto dcalc_ap : corners_->dcalcAnalysisPts()) {
        ...
        if (!graph_->arcDelayAnnotated(edge, arc, ap_index)) {
          ...
          debugPrint(debug_, "delay_calc", 3, "  %s %s -> %s %s (%s) corner:%s/%s", arc_set->from()->name(), arc->fromEdge()->asString(), arc_set->to()->name(), arc->toEdge()->asString(), arc_set->role()->asString(), dcalc_ap->corner()->name(), dcalc_ap->delayMinMax()->asString());
          debugPrint(debug_, "delay_calc", 3, "    from_slew = %s to_slew = %s", delayAsString(from_slew, this), delayAsString(to_slew, this));
          float related_out_cap = 0.0;
          if (related_out_pin)
            related_out_cap = loadCap(related_out_pin, to_rf, dcalc_ap, arc_delay_calc);
          ArcDelay check_delay = arc_delay_calc->checkDelay(to_pin, arc, from_slew, to_slew, related_out_cap, dcalc_ap);
          debugPrint(debug_, "delay_calc", 3, "    check_delay = %s", delayAsString(check_delay, this));
                                 ...
                                 const DcalcAnalysisPt *dcalc_ap)
{
  if (clk_network_->isIdealClock(from_vertex->pin()))
    return clk_network_->idealClkSlew(from_vertex->pin(), from_rf, dcalc_ap->checkClkSlewMinMax());
  else
                                ...
                                int digits)
{
  ...
  if (from_rf && to_rf) {
    ...
    if (role->isTimingCheck()) {
      ...
      result = arc_delay_calc_->reportCheckDelay(to_pin, arc, from_slew, from_slew_annotation, to_slew, related_out_cap, dcalc_ap, digits);
    }
    else {
      const Slew &from_slew = edgeFromSlew(from_vertex, from_rf, edge, dcalc_ap);
      const Parasitic *to_parasitic;
      float load_cap;
      parasiticLoad(to_pin, to_rf, dcalc_ap, nullptr, arc_delay_calc_, load_cap, to_parasitic);
      LoadPinIndexMap load_pin_index_map = makeLoadPinIndexMap(to_vertex);
      result = arc_delay_calc_->reportGateDelay(to_pin, arc, from_slew, load_cap, to_parasitic, load_pin_index_map, dcalc_ap, digits);
    }
    arc_delay_calc_->finishDrvrPin();
  }
  return result;
                          ...
                          bool &exists)
{
  ...
  for (auto dcalc_ap : corners_->dcalcAnalysisPts()) {
    ...
    bool exists1 = false;
    graph_->periodCheckAnnotation(pin, dcalc_ap->index(), min_period, exists);
    if (exists1 && (!exists || min_period1 < min_period)) {
...
MultiDrvrNet::findCaps(const Sdc *sdc)
{
  ...
  for (auto dcalc_ap : corners->dcalcAnalysisPts()) {
    ...
    for (auto drvr_rf : RiseFall::range()) {
      ...
      bool has_net_load;
      // Find pin and external pin/wire capacitance.
      sdc->connectedCap(drvr_pin, drvr_rf, corner, min_max, pin_cap, wire_cap, fanout, has_net_load);
