...
int riseFallEdgeIndex(RiseFall *from_rf, RiseFall *to_rf) {
  if (from_rf == RiseFall::fall() && to_rf == RiseFall::fall()) {
    return 0;
  } else if (from_rf == RiseFall::fall() && to_rf == RiseFall::rise()) {
    return 1;
  } else if (from_rf == RiseFall::rise() && to_rf == RiseFall::fall()) {
    return 2;
  } else if (from_rf == RiseFall::rise() && to_rf == RiseFall::rise()) {
    return 3;
  } else {
    LOG_ERROR << "Error from_rf or to_rf";
    return -1;
  }
}
