...
std::string 
strJoin(const std::vector<std::string>& elems, const std::string& deli) {
  return std::accumulate(elems.begin(), elems.end(), std::string(), 
    [&deli](const std::string& a, const std::string& b) -> std::string { 
        return a + (a.length() > 0 ? deli : "") + b; 
    } );
}

bool 
replaceLast(std::string& str, const std::string& from, const std::string& to) {
