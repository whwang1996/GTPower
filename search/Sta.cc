                 ...
                 bool infer_latches)
{
  ...
  LibertyLibrary *library = readLibertyFile(filename, corner, min_max, infer_latches);
  if (library
                      ...
                      const MinMax *min_max)
{
  ...
  LibertyLibrary::makeCornerMap(liberty, corner->libertyIndex(min_max), network_, report_);
}
                   ...
                   const char *max_filename)
{
  ...
  if (max_lib) {
    LibertyLibrary *min_lib = readLibertyFile(min_filename, cmd_corner_, MinMaxAll::min(), false);
...
Sta::setDriveCell(const LibertyLibrary *library,
                  ...
                  const MinMaxAll *min_max)
{
  sdc_->setDriveCell(library, cell, port, from_port, from_slews, to_port, rf, min_max);
  delaysInvalidFrom(port);
}
...
Sta::makeGeneratedClock(const char *name,
                        ...
                        char *comment)
{
  sdc_->makeGeneratedClock(name, pins, add_to_pins, src_pin, master_clk, divide_by, multiply_by, duty_cycle, invert, combinational, edges, edge_shifts, comment);
  update_genclks_ = true;
  search_->arrivalsInvalid();
}
...
Sta::setClockUncertainty(Clock *from_clk,
                         ...
                         float uncertainty)
{
  sdc_->setClockUncertainty(from_clk, from_rf, to_clk, to_rf, setup_hold, uncertainty);
  search_->arrivalsInvalid();
}
...
Sta::disabledEdges()
{
  ...
  while (vertex_iter.hasNext()) {
    ...
    while (edge_iter.hasNext()) {
      ...
      if (isDisabledConstant(edge) || isDisabledCondDefault(edge) || isDisabledConstraint(edge) || edge->isDisabledLoop() || isDisabledPresetClr(edge))
...
Sta::isDisabledConstraint(Edge *edge)
{
  ...
  return sdc_->isDisabled(from_pin) || sdc_->isDisabled(to_pin) || sdc_->isDisabled(inst, from_pin, to_pin, edge->role()) || sdc_->isDisabled(edge) || sdc_->isDisabled(arc_set);
}
...
Sta::isDisabledConstant(Edge *edge)
{
  ...
  const Instance *inst = network_->instance(from_pin);
  return sim_->logicZeroOne(from_vertex) || sim_->logicZeroOne(to_vertex) || (!role->isWire() && (isCondDisabled(edge, inst, from_pin, to_pin, network_, sim_) || isModeDisabled(edge, inst, network_, sim_) || isTestDisabled(inst, from_pin, to_pin, network_, sim_) || hasDisabledArcs(edge, graph_)));
}
...
Sta::disabledConstantPins(Edge *edge)
{
  ...
  if (edge->role()->isWire()) {
    ...
    isCondDisabled(edge, inst, from_pin, to_pin, network_, sim_, is_disabled, disable_cond);
    if (is_disabled)
      exprConstantPins(disable_cond, inst, pins);
    isModeDisabled(edge, inst, network_, sim_, is_disabled, disable_cond);
    if (is_disabled)
      exprConstantPins(disable_cond, inst, pins);
    Pin *scan_enable;
    isTestDisabled(inst, from_pin, to_pin, network_, sim_, is_disabled, scan_enable);
    if (is_disabled)
    ...
    if (hasDisabledArcs(edge, graph_)) {
      ...
      if (to_port) {
        ...
        if (func && sim_->functionSense(inst, from_pin, to_pin) != edge->sense())
...
{
  return !sdc_->bidirectInstPathsEnabled() && edge->isBidirectInstPath();
}

bool
Sta::isDisabledBidirectNetPath(Edge *edge) const
{
  return !sdc_->bidirectNetPathsEnabled() && edge->isBidirectNetPath();
}

bool
Sta::isDisabledPresetClr(Edge *edge) const
{
  return !sdc_->presetClrArcsEnabled() && edge->role() == TimingRole::regSetClr();
}
...
Sta::setInputDelay(const Pin *pin,
                   ...
                   float delay)
{
  sdc_->setInputDelay(pin, rf, clk, clk_rf, ref_pin, source_latency_included, network_latency_included, min_max, add, delay);
...
Sta::setOutputDelay(const Pin *pin,
                    ...
                    float delay)
{
  sdc_->setOutputDelay(pin, rf, clk, clk_rf, ref_pin, source_latency_included, network_latency_included, min_max, add, delay);
  sdcChangedGraph();
  search_->requiredInvalid(pin);
}
...
Sta::makeMulticyclePath(ExceptionFrom *from,
                        ...
                        const char *comment)
{
  sdc_->makeMulticyclePath(from, thrus, to, min_max, use_end_clk, path_multiplier, comment);
  search_->arrivalsInvalid();
}
...
Sta::makePathDelay(ExceptionFrom *from,
                   ...
                   const char *comment)
{
  sdc_->makePathDelay(from, thrus, to, min_max, ignore_clk_latency, delay, comment);
  search_->endpointsInvalid();
  search_->arrivalsInvalid();
}
...
{
  return PathGroups::isGroupPathName(group_name) || sdc_->findClock(group_name) || sdc_->isGroupPathName(group_name);
}
...
Sta::makeExceptionFrom(PinSet *from_pins,
                       ...
                       const RiseFallBoth *from_rf)
{
  return sdc_->makeExceptionFrom(from_pins, from_clks, from_insts, from_rf);
}
  ...
  if (from) {
    ...
    while (pin_iter.hasNext()) {
      ...
      if (exceptionFromInvalid(pin)) {
        if (line)
          report_->fileWarn(1554, file, line, "'%s' is not a valid start point.", cmd_network_->pathName(pin));
        else
          report_->warn(1550, "'%s' is not a valid start point.", cmd_network_->pathName(pin));
      }
    }
  }
}
...
Sta::exceptionFromInvalid(const Pin *pin) const
{
  ...
  return (net == nullptr && !network_->isTopLevelPort(pin)) || (net
                                                                // Pins connected to power/ground are invalid.
                                                                && (network_->isPower(net) || network_->isGround(net))) ||
      !((network_->isTopLevelPort(pin) && network_->direction(pin)->isAnyInput()) || network_->isRegClkPin(pin) || network_->isLatchData(pin) || network_->direction(pin)->isInternal());
}
  ...
  if (to) {
    ...
    while (pin_iter.hasNext()) {
      ...
      if (sdc_->exceptionToInvalid(pin)) {
        if (line)
          report_->fileWarn(1551, file, line, "'%s' is not a valid endpoint.", cmd_network_->pathName(pin));
        else
          report_->warn(1552, "'%s' is not a valid endpoint.", cmd_network_->pathName(pin));
      }
    }
  }
}
...
Sta::writeSdc(const char *filename,
              ...
              bool no_timestamp)
{
  sta::writeSdc(network_->topInstance(), filename, "write_sdc", leaf, native, digits, gzip, no_timestamp, sdc_);
}
                 ...
                 bool generated_clks)
{
  ...
  if (check_timing_ == nullptr)
    makeCheckTiming();
  return check_timing_->check(no_input_delay, no_output_delay, reg_multiple_clks, reg_no_clks, unconstrained_endpoints, loops, generated_clks);
}
...
Sta::setCrprEnabled(bool enabled)
{
  ...
  if (sdc_->analysisType() == AnalysisType::ocv && enabled != sdc_->crprEnabled())
    search_->arrivalsInvalid();
...
{
  // Pessimism is only relevant for on_chip_variation analysis.
  if (sdc_->analysisType() == AnalysisType::ocv && sdc_->crprEnabled() && sdc_->crprMode() != mode)
    search_->arrivalsInvalid();
...
{
  searchPreamble();
  return search_->findPathEnds(from, thrus, to, unconstrained, corner, min_max, group_count, endpoint_count, unique_pins, slack_min, slack_max, sort_by_slack, group_names, setup, hold, recovery, removal, clk_gating_setup, clk_gating_hold);
}
...
Sta::setReportPathFields(bool report_input_pin,
                         ...
                         bool report_fanout)
{
  report_path_->setReportFields(report_input_pin, report_net, report_cap, report_slew, report_fanout);
}
...
{
  clkSkewPreamble();
  clk_skews_->reportClkSkew(clks, corner, setup_hold, include_internal_latency, digits);
}
...
{
  clkSkewPreamble();
  return clk_skews_->findWorstClkSkew(cmd_corner_, setup_hold, include_internal_latency);
}
...
{
  if (!(search_->havePathGroups() && search_->arrivalsValid())) {
                            ...
                            const MinMax *min_max)
{
  ...
  while (path_iter.hasNext()) {
    ...
    if (!path->tag(this)->isGenClkSrcPath() && delayGreater(arrival, worst_arrival, min_max, this)) {
                             ...
                             const MinMax *min_max)
{
  ...
  while (path_iter.hasNext()) {
    ...
    if (!path->tag(this)->isGenClkSrcPath() && delayGreater(path_req, worst_req, req_min_max, this)) {
      worst_req = path_req;
      worst_path.init(path);
    }
  }
                ...
                const MinMax *min_max)
{
  ...
  if (bidirect_vertex) {
    Arrival arrival1 = vertexArrival(bidirect_vertex, rf, clk_edge_wildcard, nullptr, min_max);
                   ...
                   const MinMax *min_max)
{
  ...
  while (path_iter.hasNext()) {
    ...
    if ((clk_edge == clk_edge_wildcard || clk_info->clkEdge() == clk_edge) && !clk_info->isGenClkSrcPath() && delayGreater(path->arrival(this), arrival, min_max, this))
                    ...
                    const MinMax *min_max)
{
  ...
  while (path_iter.hasNext()) {
    ...
    if ((clk_edge == clk_edge_wildcard || path->clkEdge(search_) == clk_edge) && delayGreater(path_required, required, req_min_max, this))
                  ...
                  const PathAnalysisPt *path_ap)
{
  ...
  while (path_iter.hasNext()) {
    Path *path = path_iter.next();
    Slack path_slack = path->slack(this);
    if ((clk_edge == clk_edge_wildcard || path->clkEdge(search_) == clk_edge) && delayLess(path_slack, slack, this))
      slack = path_slack;
  }
  return slack;
}
...
MinPeriodEndVisitor::visit(PathEnd *path_end)
{
  ...
  if ((end_type == PathEnd::Type::check || end_type == PathEnd::Type::output_delay) && path->minMax(sta_) == MinMax::max() && src_edge->clock() == clk_ && tgt_edge->clock() == clk_
      // Only consider rise/rise and fall/fall paths.
      && src_edge->transition() == tgt_edge->transition() && path_end->multiCyclePath() == nullptr && (include_port_paths_ || !(network->isTopLevelPort(path->pin(sta_)) || pathIsFromInputPort(path_end)))) {
...
Sta::findRequired(Vertex *vertex)
{
  ...
  if (sdc_->crprEnabled() && search_->crprPathPruningEnabled() && !search_->crprApproxMissingRequireds()
      ...
      && !search_->isClock(vertex) && vertex->requiredsPruned()) {
    ...
    debugPrint(debug_, "search", 1, "resurrect pruned required %s fanout %d", vertex->name(sdc_network_), fanout);
...
{
  findDelays();
  sta::writeSdf(filename, corner, divider, include_typ, digits, gzip, no_timestamp, no_version, this);
}
                 ...
                 int &fanout)
{
  ...
  for (RiseFall *rf : RiseFall::range()) {
    ...
    sdc_->portExtCap(port, rf, corner, min_max, pin_cap1, pin_exists1, wire_cap1, wire_exists1, fanout1, fanout_exists1);
    if (pin_exists1) {
              ...
              bool reduce)
{
  ...
  bool success = readSpefFile(filename, instance, ap, pin_cap_included, keep_coupling_caps, coupling_cap_factor, reduce, corner, min_max, this);
  delaysInvalid();
  return success;
}
                              ...
                              const LibertyCell *to_cell)
{
  ...
  if (to_port == nullptr || (!libertyPortCapsEqual(to_port, from_port)
...
libertyPortCapsEqual(const LibertyPort *port1,
                     const LibertyPort *port2)
{
  return port1->capacitance(RiseFall::rise(), MinMax::min()) == port2->capacitance(RiseFall::rise(), MinMax::min()) && port1->capacitance(RiseFall::rise(), MinMax::max()) == port2->capacitance(RiseFall::rise(), MinMax::max()) && port1->capacitance(RiseFall::fall(), MinMax::min()) == port2->capacitance(RiseFall::fall(), MinMax::min()) && port1->capacitance(RiseFall::fall(), MinMax::max()) == port2->capacitance(RiseFall::fall(), MinMax::max());
}
...
{
  return (thru_disabled_ || !from_vertex->isDisabledConstraint()) && (thru_constants_ || !from_vertex->isConstant());
}
...
FanInOutSrchPred::searchThru(Edge *edge)
{
  ...
  return searchThruRole(edge) && (thru_disabled_ || !(edge->isDisabledConstraint() || edge->isDisabledCond() || sdc->isDisabledCondDefault(edge))) && (thru_constants_ || edge->simTimingSense() != TimingSense::none);
}
...
FanInOutSrchPred::searchThruRole(Edge *edge)
{
  ...
  return role == TimingRole::wire() || role == TimingRole::combinational() || role == TimingRole::tristateEnable() || role == TimingRole::tristateDisable();
}
...
{
  return (thru_disabled_ || !to_vertex->isDisabledConstraint()) && (thru_constants_ || !to_vertex->isConstant());
}
...
{
  TimingRole *role = edge->role();
  return role == TimingRole::wire() || role == TimingRole::combinational() || role == TimingRole::tristateEnable() || role == TimingRole::tristateDisable() || role == TimingRole::regClkToQ() || role == TimingRole::latchEnToQ();
}
                   ...
                   bool thru_constants)
{
  ...
  while (to_iter.hasNext()) {
    ...
    if (network_->isHierarchical(pin)) {
      ...
      while (edge_iter.hasNext()) {
        Edge *edge = edge_iter.next();
        findFaninPins(edge->from(graph_), flat, startpoints_only, inst_levels, pin_levels, fanin, pred);
      }
    }
    else {
      Vertex *vertex = graph_->pinLoadVertex(pin);
      findFaninPins(vertex, flat, startpoints_only, inst_levels, pin_levels, fanin, pred);
    }
  }
  return fanin;
}
                   ...
                   SearchPred &pred)
{
  ...
  findFaninPins(vertex, flat, inst_levels, pin_levels, visited, &pred, 0, 0);
  ...
  while (visited_iter.hasNext()) {
    ...
    if (!startpoints_only || network_->isRegClkPin(visited_pin) || !hasFanin(visited_vertex, &pred, graph_))
...
Sta::findFaninPins(Vertex *to,
                   ...
                   int pin_level)
{
  debugPrint(debug_, "fanin", 1, "%s", to->name(sdc_network_));
  if (!visited.hasKey(to)) {
    visited.insert(to);
    Pin *to_pin = to->pin();
    bool is_reg_clk_pin = network_->isRegClkPin(to_pin);
    if (!is_reg_clk_pin && (inst_levels <= 0 || inst_level < inst_levels) && (pin_levels <= 0 || pin_level < pin_levels) && pred->searchTo(to)) {
      VertexInEdgeIterator edge_iter(to, graph_);
      while (edge_iter.hasNext()) {
        Edge *edge = edge_iter.next();
        Vertex *from_vertex = edge->from(graph_);
        if (pred->searchThru(edge) && (flat || !crossesHierarchy(edge)) && pred->searchFrom(from_vertex)) {
          findFaninPins(from_vertex, flat, inst_levels, pin_levels, visited, pred, edge->role()->isWire() ? inst_level : inst_level + 1, pin_level + 1);
        }
      }
    }
  }
...
Sta::findFaninInstances(PinSeq *to,
                        ...
                        bool thru_constants)
{
  PinSet pins = findFaninPins(to, flat, startpoints_only, inst_levels, pin_levels, thru_disabled, thru_constants);
                    ...
                    bool thru_constants)
{
  ...
  while (from_iter.hasNext()) {
    ...
    if (network_->isHierarchical(pin)) {
      ...
      while (edge_iter.hasNext()) {
        Edge *edge = edge_iter.next();
        findFanoutPins(edge->to(graph_), flat, endpoints_only, inst_levels, pin_levels, fanout, pred);
      }
    }
    else {
      Vertex *vertex = graph_->pinDrvrVertex(pin);
      findFanoutPins(vertex, flat, endpoints_only, inst_levels, pin_levels, fanout, pred);
    }
  }
  return fanout;
}
...
{
  VertexSet visited(graph_);
  findFanoutPins(vertex, flat, inst_levels, pin_levels, visited, &pred, 0, 0);
  VertexSet::Iterator visited_iter(visited);
  while (visited_iter.hasNext()) {
    Vertex *visited_vertex = visited_iter.next();
    Pin *visited_pin = visited_vertex->pin();
    if (!endpoints_only || search_->isEndpoint(visited_vertex, &pred))
...
Sta::findFanoutPins(Vertex *from,
                    ...
                    int pin_level)
{
  debugPrint(debug_, "fanout", 1, "%s", from->name(sdc_network_));
  if (!visited.hasKey(from)) {
    visited.insert(from);
    if (!search_->isEndpoint(from, pred) && (inst_levels <= 0 || inst_level < inst_levels) && (pin_levels <= 0 || pin_level < pin_levels) && pred->searchFrom(from)) {
      VertexOutEdgeIterator edge_iter(from, graph_);
      while (edge_iter.hasNext()) {
        Edge *edge = edge_iter.next();
        Vertex *to_vertex = edge->to(graph_);
        if (pred->searchThru(edge) && (flat || !crossesHierarchy(edge)) && pred->searchTo(to_vertex)) {
          findFanoutPins(to_vertex, flat, inst_levels, pin_levels, visited, pred, edge->role()->isWire() ? inst_level : inst_level + 1, pin_level + 1);
        }
      }
    }
  }
...
Sta::findFanoutInstances(PinSeq *from,
                         ...
                         bool thru_constants)
{
  PinSet pins = findFanoutPins(from, flat, endpoints_only, inst_levels, pin_levels, thru_disabled, thru_constants);
  return pinInstances(pins, network_);
}
...
Sta::slowDrivers(int count)
{
  ...
  sort(insts, [=](const Instance *inst1, const Instance *inst2) {
                          ...
                          const MinMax *min_max)
{
  ...
  Slew slew1;
  float limit1, slack1;
  check_slew_limits_->checkSlew(pin, corner, min_max, true, corner1, rf1, slew1, limit1, slack1);
  report_path_->reportLimitShort(report_path_->fieldSlew(), pin, delayAsFloat(slew1), limit1, slack1);
}
                            ...
                            const MinMax *min_max)
{
  ...
  float limit1, slack1;
  check_slew_limits_->checkSlew(pin, corner, min_max, true, corner1, rf1, slew1, limit1, slack1);
  report_path_->reportLimitVerbose(report_path_->fieldSlew(), pin, rf1, delayAsFloat(slew1), limit1, slack1, corner1, min_max);
}
...
Sta::checkSlew(const Pin *pin,
               ...
               float &slack)
{
  check_slew_limits_->checkSlew(pin, corner, min_max, check_clks, corner1, rf, slew, limit, slack);
}
...
{
  checkSlewLimitPreamble();
  PinSeq pins = check_slew_limits_->checkSlewLimits(nullptr, false, nullptr, MinMax::max());
  pin = nullptr;
  slew = 0.0;
  slack = INF;
  limit = INF;
  if (!pins.empty()) {
    ...
    const Corner *corner;
    const RiseFall *rf;
    check_slew_limits_->checkSlew(pin, nullptr, MinMax::max(), true, corner, rf, slew, limit, slack);
  }
}
                   ...
                   bool &exists)
{
  if (check_slew_limits_ == nullptr)
    makeCheckSlewLimits();
  check_slew_limits_->findLimit(port, corner, min_max, limit, exists);
}
                            ...
                            const MinMax *min_max)
{
  ...
  check_fanout_limits_->checkFanout(pin, min_max, fanout, limit, slack);
    ...
    float &limit)
{
  ...
  if (!pins.empty()) {
    pin = pins[0];
    check_fanout_limits_->checkFanout(pin, MinMax::max(), fanout, limit, slack);
  }
}
                            ...
                            const MinMax *min_max)
{
  ...
  return check_capacitance_limits_->checkCapacitanceLimits(net, violators, corner, min_max);
}
                                 ...
                                 const MinMax *min_max)
{
  ...
  check_capacitance_limits_->checkCapacitance(pin, corner, min_max, corner1, rf, capacitance, limit, slack);
                                   ...
                                   const MinMax *min_max)
{
  ...
  check_capacitance_limits_->checkCapacitance(pin, corner, min_max, corner1, rf1, capacitance1, limit1, slack1);
...
{
  checkCapacitanceLimitPreamble();
  PinSeq pins = check_capacitance_limits_->checkCapacitanceLimits(nullptr, false, nullptr, MinMax::max());
  pin = nullptr;
  capacitance = 0.0;
  slack = INF;
  limit = INF;
  if (!pins.empty()) {
    ...
    const Corner *corner;
    const RiseFall *rf;
    check_capacitance_limits_->checkCapacitance(pin, nullptr, MinMax::max(), corner, rf, capacitance, limit, slack);
  }
}
...
Sta::writeTimingModel(const char *lib_name,
                      ...
                      const Corner *corner)
{
  LibertyLibrary *library = makeTimingModel(lib_name, cell_name, filename, corner, this);
...
Sta::powerPreamble()
{
  utils::ScopedTimer sta_timer("STA");
  TIMERSTART(STA);
...
Sta::printPowerAnalysisRes(const PowerResult &result) const
{
  ...
  utils::create_log_and_res_dir();
  LOG_INFO << "************" << "\n"
    << std::scientific << std::setprecision(3)
    << "Total Internal power: " << result.internal() << "\n"
    << "Total switching power: " << result.switching() << "\n"
    << "Leakage power: " << result.leakage() << "\n"
    << "Total glitch power: " << result.glitchInternal() + result.glitchSwitching();
  LOG_INFO << "************" << "\n"
    << std::scientific << std::setprecision(3)
    << "Glitch internal power: " << result.glitchInternal() << "\n"
    << "Glitch switching power: " << result.glitchSwitching();
  LOG_INFO << "Clock period for power calculation: " << power_->clkPeriod();
  if (G_CONFIG.flags.enable_time_based_analysis) {
    result.printPerCycleResults(power_->network());
    power_->printCellRes();
  }

  TIMEREND(PRINT_POWER_RES);
  DURATION_ms(PRINT_POWER_RES);
}
